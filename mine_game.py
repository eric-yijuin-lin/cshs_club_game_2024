...
class MineMapCell:
  ...
  def roll_cell(self) -> None:
    if uniform(0, 1) > RESOURCE_CELL_RATE:
      self.recource_type = ResourceType.Nothing
    else:
      type_count = len(ResourceType)
      self.recource_type = ResourceType(randint(1, type_count - 1)) # 0 = nothing, last = n -1
      self.roll_resource_level()

  def roll_resource(self) -> None:
    num_recoures = len(ResourceType)
    self.recource_type = ResourceType(randint(1, num_recoures))
    self.roll_resource_level()
